	...
	if ((params != NULL) && (params->Status == LORAMAC_HANDLER_SUCCESS))
	  {
	    ...
	    stb_join_flag = true;
		joEmTriggerRedLedWd(LED_ON_TIME_SIG);	// Short RGB
	    joEmTriggerBlueLed(); // Success!
	    joEmTriggerGreenLed();
	  }
	  else
	  {
...
#if 0 // nachfolgend AutocodeDeaktiviert
...
ATEerror_t AT_LocalTime_get(const char *param)
{
  ...
  // UnixEpoch.Seconds += 3600 * 2; /*adding 2 hours* // Entfernt!

  SysTimeLocalTime(UnixEpoch.Seconds,  & localtime);
  // Einfacher zum Parsen - TS:UnixTimestamp
  AT_PRINTF("LTIME: TS:%u %02d:%02d:%02d %02d.%02d.%04d\r\n",
		    UnixEpoch.Seconds,
		    localtime.tm_hour, localtime.tm_min, localtime.tm_sec,
            localtime.tm_mday, localtime.tm_mon + 1, localtime.tm_year + 1900);

  return AT_OK;
#if 0 // NoUse MX-Code entfernt
...
// Reply: A:<appStats> L:<linkState>
ATEerror_t AT_Network_Join_State_get(const char *param)
{
  AT_PRINTF("A:%u L:%u\r\n",stb_join_flag,(LmHandlerJoinStatus() == LORAMAC_HANDLER_SET));
  return AT_OK;
}
