...
static const struct ATCommand_s ATCommand[] =
{
  ...
  /* USER CODE BEGIN ATCommand */
	...
	{
		.string = AT_TIMEREQUEST,
		.size_string = sizeof(AT_TIMEREQUEST) - 1,
#ifndef NO_HELP // Compiler macht ansonsten 'n Error:
		.help_string = "AT+TIMEREQ. Piggyback a Time Request to the next uplink\r\n",
#endif /* !NO_HELP */
		.get = AT_return_error,
		.set = AT_return_error,
		.run = AT_Time_Request,
...
#ifndef NO_HELP // Compiler macht sonst Error - Achtung: ein gespeicherter Kontext gilt als "Joined"! (internal, z.B. ABP)
		.help_string = "AT+NJS Get Network Join state A:<appState> L:<internal>\r\n",
#endif /* !NO_HELP */
		.get = AT_Network_Join_State_get,
		.set = AT_return_error,
		.run = AT_return_error,
	}
